...
typedef tarea_controll_block_t *task_handler_t;
...
osa_status_t OSA_TaskSetPriority(osa_task_handle_t taskHandle, osa_task_priority_t taskPriority)
{
    ...
    tarea_controll_block_t *tcb = NULL;
#if (defined(GENERIC_LIST_LIGHT) && (GENERIC_LIST_LIGHT > 0U))
    tarea_controll_block_t *preTcb = NULL;
#endif
    tarea_controll_block_t *ptaskStruct = tarea_controlol_block_t *)taskHandle;
    uint32_t regPrimask;
    ...
    {
        tcb = (tarea_controll_block_t *)(void *)list_element;
...
osa_status_t OSA_TaskCreate(osa_task_handle_t taskHandle, const osa_task_def_t *thread_def, osa_task_param_t task_param)
{
    ...
    tarea_controll_block_t *ptaskStruct = tarea_controlol_block_t *)taskHandle;
    uint32_t regPrimask;
    assert(sizeof(tarea_controll_block_t) == OSA_TASK_HANDLE_SIZE);
    assert(taskHandle);
